...
查看实际输出：**[Harvey Series A Deck](examples/harvey-series-a/)** — 一套 19 页的 pitch deck，由 `founder-pitch-deck` 生成，风格参考 Harvey AI（$110 亿法律 AI 公司）。下载 `index.html` 用浏览器打开即可（密码：`harvey`）。
